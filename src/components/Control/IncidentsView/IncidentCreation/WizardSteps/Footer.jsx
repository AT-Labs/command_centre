...
export const Footer = (props) => {
    const getColumnClass = (baseClass) => {
        if (props.isAddEffectsStep) return `col-2${baseClass}`;
        if (props.useDraftDisruptions && props.isDraftOrCreateMode) return `col-3${baseClass}`;
        return `col-4${baseClass}`;
    };

    return (
        <footer className={ `row m-0 justify-content-between p-4 position-fixed incident-footer-min-height ${props.showFinishButton && props.isAddEffectsStep ? 'footer-with-finish-button' : ''}` }>
            ...
            { (props.useDraftDisruptions && props.isDraftOrCreateMode) && (
                <div className={ props.isAddEffectsStep ? 'col-2 pl-0' : 'col-3 pl-0' }>
                    <Button
