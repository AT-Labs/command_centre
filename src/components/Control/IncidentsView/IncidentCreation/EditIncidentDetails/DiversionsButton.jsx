...
import { openDiversionManager, updateDiversionMode, updateDiversionToEdit, fetchDiversions } from '../../../../../redux/actions/control/diversions';
...
const DiversionsButton = ({
    ...
    useEffect(() => {
        if (disruption?.disruptionId && fetchDiversions) {
            fetchDiversions(disruption.disruptionId);
        }
    }, [disruption?.disruptionId, fetchDiversions]);

    // Refresh diversions when DiversionManager closes (after creating a diversion)
    useEffect(() => {
        if (!isDiversionManagerOpen && disruption?.disruptionId && fetchDiversions) {
            // Add a small delay to ensure the diversion was created
            setTimeout(() => {
                fetchDiversions(disruption.disruptionId);
            }, 500);
        }
    }, [isDiversionManagerOpen, disruption?.disruptionId, fetchDiversions]);
    ...
    const isAddDiversionEnabled = () => {
        ...
        const busRoutesWithDiversions = busRoutes.filter(route => {
            return existingDiversions.some(diversion => {
    ...
    return (
        <div className="diversions-button-container">
            <Button
                ...
                disabled={false}
            >
                <span style={{ color: '#000000' }}>Diversions({diversionCount})</span>
                <img 
                    src={detourIcon} 
                    alt="detour" 
                    width="26" 
...
const mapDispatchToProps = (dispatch) => ({
    ...
    fetchDiversions: (disruptionId) => dispatch(fetchDiversions(disruptionId)),
});
