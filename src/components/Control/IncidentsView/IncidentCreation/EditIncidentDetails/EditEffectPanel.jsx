...
import { updateDiversionMode, updateDiversionToEdit, openDiversionManager } from '../../../../../redux/actions/control/diversions';
...
export const EditEffectPanel = (props) => {
    ...
    const [isLoadingDiversions, setIsLoadingDiversions] = useState(false);
    ...
    useEffect(() => {
        const fetchDiversions = async () => {
            if (disruption?.disruptionId) {
                // First check if diversions are already in the disruption data
                if (disruption.diversions && disruption.diversions.length > 0) {
                    setDiversions(disruption.diversions);
                } else if (props.disruptions?.[0]?.diversions && props.disruptions[0].diversions.length > 0) {
                    setDiversions(props.disruptions[0].diversions);
                } else {
    ...
    const refreshDiversions = async () => {
        if (disruption?.disruptionId) {
            setIsLoadingDiversions(true);
            try {
                const diversionsData = await getDiversion(disruption.disruptionId);
                setDiversions(diversionsData || []);
            } catch (error) {
    ...
    useEffect(() => {
        if (disruptionIncidentNoToEdit && disruptions && Array.isArray(disruptions)) {
            ...
            if (disruptionToSet) {
                ...
                // If disruption not found, set a default one
                        ...
                        <div className="label-with-icon">
                            ...
                            <div className="diversions-button-container">
                                <DiversionsButton 
                                    disruption={{
                                        ...safeDisruption,
                                        diversions: diversions // Use the local diversions state directly
                                    }}
                                    ...
                                    refreshDiversions={refreshDiversions}
                                />
                            </div>
