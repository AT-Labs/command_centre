...
import DisruptionDetail from '../DisruptionsView/DisruptionDetail';
...
const getViewDisruptionDetailsButton = row => (
    [
        <Tooltip title="Open & Edit Incidents" placement="top-end" key={ uniqueId(row.disruptionId) }>
            <IconButton aria-label="open-edit-incidents"
                onClick={ () => {
                    window.open(`/control-main-view/control-incidents/${row.disruptionId.toString()}`, '_blank');
                } }>
                <BsPencilSquare />
            </IconButton>
        </Tooltip>,
