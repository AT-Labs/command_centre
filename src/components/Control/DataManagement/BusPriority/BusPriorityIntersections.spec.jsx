...
describe('<BusPriorityIntersectionsDataGrid />', () => {
    ...
    const findModals = () => {
        ...
        const travelTimeInput = withinModal.getByTestId('travel-time-input');
        const geofenceInput = withinModal.getByTestId('geofence-radius-input');
        return { travelTimeInput, geofenceInput };
    };
    ...
    it('shows alert when Geofence Radius is not a positive integer', () => {
        render(<BusPriorityIntersectionsDataGrid { ...defaultProps } />);
        const { geofenceInput } = findModals();
    ...
    it('shows alert and disables Save button when Travel Time is not a positive integer', () => {
        render(<BusPriorityIntersectionsDataGrid { ...defaultProps } />);
        const { travelTimeInput } = findModals();

        fireEvent.change(travelTimeInput, { target: { value: '0' } });

        expect(screen.getByText('Travel Time should be positive integer')).toBeInTheDocument();
        expect(screen.getByTestId('modal-save')).toBeDisabled();
    });

    it('enables Save button only when both fields are valid positive integers', () => {
        render(<BusPriorityIntersectionsDataGrid { ...defaultProps } />);
        const { travelTimeInput, geofenceInput } = findModals();

        fireEvent.change(travelTimeInput, { target: { value: '60' } });
        fireEvent.change(geofenceInput, { target: { value: '180' } });
    ...
    it('disables Save button if either field is invalid', () => {
        ...
        fireEvent.change(travelTimeInput, { target: { value: '60' } });
        fireEvent.change(geofenceInput, { target: { value: '-5' } });
    ...
    it('saves changes with updated Travel Time and Geofence Radius and closes modal', () => {
        ...
        fireEvent.change(travelTimeInput, { target: { value: '75' } });
        fireEvent.change(geofenceInput, { target: { value: '200' } });
    ...
    it('shows only the correct alert when only one field is invalid', () => {
        ...
        fireEvent.change(travelTimeInput, { target: { value: '-5' } });
        fireEvent.change(geofenceInput, { target: { value: '200' } });

        expect(screen.getByText('Travel Time should be positive integer')).toBeInTheDocument();
        expect(screen.queryByText('Geofence Radius should be positive integer')).not.toBeInTheDocument();
        expect(screen.getByTestId('modal-save')).toBeDisabled();
    });
    ...
    it('hides both alerts when both fields are valid positive integers', () => {
        ...
        expect(screen.queryByText('Geofence Radius should be positive integer')).not.toBeInTheDocument();
        expect(screen.queryByText('Travel Time should be positive integer')).not.toBeInTheDocument();
        expect(screen.getByTestId('modal-save')).not.toBeDisabled();
    });
    ...
    it('treats decimal values as invalid', () => {
        ...
        fireEvent.change(geofenceInput, { target: { value: '12.5' } });
    ...
    it('treats empty input as invalid and disables Save', () => {
        ...
        fireEvent.change(travelTimeInput, { target: { value: '' } });
