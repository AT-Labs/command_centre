...
export const DisruptionDetailSelect = (props) => {
    const { value, options, label, id, onChange } = props;
    return (
        <FormGroup className={ `${props.className}` }>
            <Label for={ id }><span className="font-size-md font-weight-bold">{label}</span></Label>
            <Input type="select" className="w-100 border border-dark" id={ id } defaultValue={ value } onChange={ e => onChange(e.currentTarget.value) }>
                {options.map((item) => {
