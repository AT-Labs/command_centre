...
export const SelectEffectEntities = (props) => {
    const { index, affectedEntities, disruptionType, disruptionId } = props;
    ...
    // const saveStopsState = stops => console.log('saveStopsState'); // props.updateAffectedStopsState(sortBy(stops, sortedStop => sortedStop.stopCode));

    const saveStopsState = stops => props.onAffectedEntitiesUpdate(index, 'affectedStops', sortBy(stops, sortedStop => sortedStop.stopCode));
    ...
    useEffect(() => {
        ...
        // onAffectedEntitiesUpdate(index, 'affectedRoutes', [...allSingleEntities, ...stopGroupStops]);
    ...
    const deselectAllEntities = () => {
        /*  props.onAffectedEntitiesUpdate(index, 'affectedStops', []);
        props.onAffectedEntitiesUpdate(index, 'affectedRoutes', []); */
        setAreEntitiesSelected(false);
        setSelectedEntities([]);
        props.updateDisruptionData(index, 'affectedEntities', {
            affectedRoutes: [],
            affectedStops: [],
        });
        /* setAffectedStopGroups([]);
        setAffectedSingleStops([]); */
        // props.deleteAffectedEntities();
    };

    const removeNotFoundFromStopGroups = () => {
        const filterStops = affectedEntities.affectedStops.filter(stop => stop.stopCode !== 'Not Found');
        if (filterStops.length !== affectedEntities.affectedStops.length) {
    ...
    const onChange = (selectedItems) => {
        ...
        if (routes.length !== affectedEntities.affectedRoutes.length) {
            ...
            props.onAffectedEntitiesUpdate(index, 'affectedRoutes', routes);
    ...
    const updateAffectedStops = (stops) => {
        props.onAffectedEntitiesUpdate(index, 'affectedStops', stops);
    };
    ...
    const toggleDisruptionType = () => {
        ...
        /* if (disruptionType === DISRUPTION_TYPE.ROUTES) {
            props.updateDisruptionData(index, 'disruptionType', DISRUPTION_TYPE.STOPS);
        } else {
            props.updateDisruptionData(index, 'disruptionType', DISRUPTION_TYPE.ROUTES);
        } */
    ...
    return (
        <div className="select_disruption">
            <RadioButtons
                key={ `${disruptionId}_radio_buttons` }
                title=""
            ...
            <div className="selection-container h-100">
                <ul className="p-0">
                    <StopsByRouteMultiSelect
                        key={ `${disruptionId}_stops_by_route_multi_select` }
                    ...
                    <RoutesByStopMultiSelect
                        key={ `${disruptionId}_routes_by_stop_multi_select` }
                    ...
                    <StopGroupsMultiSelect 
                        key={ `${disruptionId}_stop_groups_multi_select` }
                        affectedStops={ affectedEntities.affectedStops }
...
SelectEffectEntities.propTypes = {
    ...
    updateDisruptionData: PropTypes.func.isRequired,
