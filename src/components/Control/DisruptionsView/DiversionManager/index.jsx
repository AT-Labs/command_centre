...
const DiversionManager = (props) => {
    ...
    React.useEffect(() => {
        const MAP_ELEMENTS_SELECTOR = '.leaflet-control-container, .leaflet-control-zoom, .leaflet-control-draw, '
        ...
        const toggleMapElements = (isVisible) => {
            const mapElements = document.querySelectorAll(MAP_ELEMENTS_SELECTOR);
            const mapContainer = document.querySelector(MAP_CONTAINER_SELECTOR);

            const styleValues = isVisible ? { display: '', visibility: '', opacity: '' } : { display: 'none', visibility: 'hidden', opacity: '0' };

            mapElements.forEach((element) => {
                if (element) {
                    Object.assign(element.style, styleValues);
                }
            });

            if (mapContainer) {
                Object.assign(mapContainer.style, styleValues);
            }
        };

        const handleCloseButtonClick = () => toggleMapElements(false);
        const handleOpenButtonClick = () => toggleMapElements(true);

        document.addEventListener('closeButtonClick', handleCloseButtonClick);
        document.addEventListener('openButtonClick', handleOpenButtonClick);

        return () => {
            document.removeEventListener('closeButtonClick', handleCloseButtonClick);
            document.removeEventListener('openButtonClick', handleOpenButtonClick);
        };
    }, []);
    ...
    const handleOnlyOneRouteVariantCB = (checked) => {
        ...
        if (checked) {
            ...
            if (availableRouteVariants.length > 0) {
                ...
                // Hide all as we automatically select all available route variants
            } else {
                setSelectedOtherRouteVariants([]);
