...
describe('<SelectDetails />', () => {
    ...
    describe('onChangeEndDate logic', () => {
        let data;
        let stops;
        let routes;
        let setCssEndDateInvalidStub;
        ...
        beforeEach(() => {
            ...
            setCssEndDateInvalidStub = sandbox.stub();
            sandbox.stub(React, 'useState').callsFake((initialValue) => {
                if (initialValue === '') {
                    return ['', setCssEndDateInvalidStub];
                }
                return [initialValue, () => {}];
            });
        });

        it('should set endTime to 23:59 when endDateValue is set and endTime is empty', () => {
            const mockData = {
                ...data,
                endTime: '',
                recurrent: false,
            ...
            if (flatpickr.length > 0) {
                ...
                expect(componentPropsMock.onDataUpdate).toHaveBeenCalledWith('endDate', '10/03/2022');
                expect(componentPropsMock.onDataUpdate).toHaveBeenCalledWith('endTime', '23:59');
                expect(setCssEndDateInvalidStub).toHaveBeenCalledWith('');
            }
        });

        it('should not set endTime when endDateValue is empty', () => {
            const mockData = {
                ...data,
                endTime: '',
                recurrent: false,
            ...
            if (flatpickr.length > 0) {
                ...
                expect(componentPropsMock.onDataUpdate).toHaveBeenCalledWith('endDate', '');
                expect(componentPropsMock.onDataUpdate).not.toHaveBeenCalledWith('endTime', '23:59');
                expect(setCssEndDateInvalidStub).toHaveBeenCalledWith('');
            }
        });

        it('should not set endTime when endTime is already set', () => {
            const mockData = {
                ...data,
                endTime: '12:00',
                recurrent: false,
