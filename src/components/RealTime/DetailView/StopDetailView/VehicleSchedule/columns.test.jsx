...
describe('Vehicle schedule columns', () => {
    context('formatDestination()', () => {
        ...
        it('should format train with Sylvia Park destinationDisplay', () => {
            const isTrainStop = true;
            const destinationDisplay = 'Sylvia Park via Panmure';
            const formattedDestination = formatDestination(isTrainStop, destinationDisplay);
            expect(formattedDestination.type).to.equal('div');
            expect(formattedDestination.props.children[0].props.children).to.equal('Sylvia Park');
            expect(formattedDestination.props.children[1].props.children).to.equal(' via Panmure');
        });
    });
